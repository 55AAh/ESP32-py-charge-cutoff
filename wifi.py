    ...
    async def ensure_connected(cls):
        while not cls._connected:
            try:
                ...
                log.error("%s; sleeping for %d s before retrying...", repr(e.args), Utils.wifi_retry_sleep_time)
    ...
    async def retry_run_task(cls, task):
        while True:
            ...
            try:
                ...
                log.error("Sleeping for %d s before retrying...", Utils.wifi_retry_sleep_time)
                await asyncio.sleep(Utils.wifi_retry_sleep_time)

    @classmethod
    async def request(cls, method: str, url: str, headers: dict | None = None, json_body: dict | None = None):
        await asyncio.sleep(0.1)
        gc.collect()
        try:
            with Led.Blink:
